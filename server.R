...
print(paste0("Time to load data: ",Sys.time() - start.time))
...
shinyServer(function(input, output, session) {
  ...
  observe({
    updateSelectizeInput(session,'chooseFilter2', selected = "None",
  ...
  observe({
    updateSelectizeInput(session,'chooseFilter3', selected = "None",
  ...
  observeEvent(input$do,{
    print(paste0("Table - ",
                 input$chooseChapter, " - ",
                 input$chooseSection, " - ",
                 input$chooseQuestion, " - ",
                 input$chooseDemographic, " - ",
                 input$chooseFilter1Option, " - ",
                 input$chooseFilter2, " - ",
                 input$chooseFilter2Option, " - ",
                 input$chooseFilter3, " - ",
                 input$chooseFilter3Option))
  })

  
  output$glossary <- renderTable({Glossary})
  output$terminology <- renderTable(colnames = FALSE, {Terminology})
  ...
  description <-  eventReactive(input$do,{
    description <- paste0("<h5> <b> ",input$chooseChapter," - ",input$chooseSection,"</b> <br>",
           "This question covers <b>",input$chooseQuestion,"</b> by <b>",input$chooseDemographic, "</b>",
           "<p> 
           <br>
           You have chosen to focus on <b>",input$chooseFilter1Option,"</b> respondents. <br> 
  ...
  notes <-  eventReactive(input$do,{ 
    ...
    if(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes1"] == "No notes"){
        Notes1 <- ""} else{ Notes1 <- paste(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes1"])}
      
    if(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes2"] == "No notes"){
        Notes2 <- ""} else{ Notes2 <- paste(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes2"])}
      
    if(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes3"] == "No notes"){
        Notes3 <- ""} else{ Notes3 <- paste(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes3"])}
  ...
  output$downloadData <-  downloadHandler(filename = function() {
    paste('LGBTSurvey - ',input$`chooseSub-theme`,' - ', Sys.time(), '.csv', sep="")
  },
  content = function(file) {
    print(paste0("Download - ",
                 input$chooseChapter, " - ",
                 input$chooseSection, " - ",
                 input$chooseQuestion, " - ",
                 input$chooseDemographic, " - ",
                 ...
                 input$chooseFilter3, " - ",
                 input$chooseFilter3Option))
    write.csv(outputlist()$download, file, quote=FALSE,row.names = FALSE)}
  )
  ...
  outputlist  <- eventReactive(input$do,{
       ...
       w.data <- data %>%
          ...
          w.data <- w.data %>% collect()
                ...
                names(w.data)[names(w.data) == "Rows"] <- "Responses"
          
         
          
          if(nrow(w.data) > 0 & w.data$value[1] != "No Data available"){w.data[w.data$Columns == "Row Totals",]$Columns <- "All respondents"}
    ...
    if(w.data$value[1] == "No Data available"){
      return(list(datatest="No Data", dataout=NULL, stackedplot=NULL,download=NULL))}
          ...
          roworder <- unique(w.data$Responses)
   
   
   datatest <- TRUE   
   
   table <- dcast(w.data, Responses ~ Columns, value.var="value")
   table <- table[match(roworder, table$Responses),]
   table <- table[, c("Responses",colorder)]
   download <- table
   ...
   if(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes1"] == "No notes"){
     DownloadNotes1 <- ""} else{ DownloadNotes1 <- paste(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes1"])}
   
   if(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes2"] == "No notes"){
     DownloadNotes2 <- ""} else{ DownloadNotes2 <- paste(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes2"])}
   
   if(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes3"] == "No notes"){
     DownloadNotes3 <- ""} else{ DownloadNotes3 <- paste(vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"Notes3"])}
   
 
   
   notesdownload <- if(DownloadNotes1 == "" & DownloadNotes2 == "" & DownloadNotes3 == ""){
       matrix(c(
         rep("",ncol(table)),
         "There are no notes for the chosen options",
         rep("",ncol(table)-1)),nrow=2,byrow=TRUE)} else {
       matrix(c("","Notes","",DownloadNotes1,DownloadNotes2,DownloadNotes3         ,rep("", 6*(ncol(table)-1))),nrow=6,byrow=FALSE)}
   ...
   download <-
     ...
     "All / Trans / Cisgender", "Category",  "Filter 1",
     "Filter 1 Option",         "Filter 2",  "Filter 2 Options",
     "", "Question Wording", "",
     input$chooseChapter, input$chooseSection, input$chooseQuestion,
     input$chooseFilter1Option, input$chooseDemographic, input$chooseFilter2,
     input$chooseFilter2Option, input$chooseFilter3, input$chooseFilter3Option,
     "", vlookup[vlookup$Section == input$chooseSection & vlookup$VariableName == input$chooseQuestion,"VariableDescription"], "",
     rep("",12*(ncol(table)-2))),nrow=12,byrow = FALSE),
     colnames(download)),
     as.matrix(download)),
     notesdownload)
   
     colnames(download) <- rep("",ncol(download))
  ...
  w.data$Responses <- factor(w.data$Responses, levels = rev(roworder))
  ...
  categories <- categories[s,c("Responses")]
  
  if(length(s) > 0){w.data <- w.data[w.data$Responses %in% categories,]}
  w.data <- w.data[w.data$Responses != "Totals",]
    
  pallete_no <- length(unique(as.character(w.data$Responses)))
  pallete <- rev(get(paste0("lgbt_pallete_",pallete_no)))
  
  stackedplot <- ggplot(data = w.data, aes(x = Columns, y = as.numeric(value), fill = Responses)) +
...
lgbt_pallete_0 <- c(rgb(16,79,117,maxColorValue=255))

lgbt_pallete_1 <- c(rgb(16,79,117,maxColorValue=255))

lgbt_pallete_2 <- c(rgb(16,79,117,maxColorValue=255),
rgb(112,149,172,maxColorValue=255))

lgbt_pallete_3 <- c(rgb(16,79,117,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(207,220,227,maxColorValue=255))

lgbt_pallete_4 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255))

lgbt_pallete_5 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255))

lgbt_pallete_6 <- c(rgb(16,79,117,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
rgb(138,37,41,maxColorValue=255),
rgb(185,124,127,maxColorValue=255),
rgb(232,211,212,maxColorValue=255))

lgbt_pallete_7 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(138,37,41,maxColorValue=255),
...
lgbt_pallete_8 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(138,37,41,maxColorValue=255),
rgb(161,81,84,maxColorValue=255),
rgb(185,124,127,maxColorValue=255),
rgb(208,168,169,maxColorValue=255))

lgbt_pallete_9 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_10 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_11 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(138,37,41,maxColorValue=255),
...
lgbt_pallete_12 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(138,37,41,maxColorValue=255),
...
lgbt_pallete_13 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_14 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_15 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_16 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(138,37,41,maxColorValue=255),
...
rgb(241,177,120,maxColorValue=255),
rgb(246,203,165,maxColorValue=255),
rgb(0,71,18,maxColorValue=255),
rgb(51,108,65,maxColorValue=255),
rgb(102,145,113,maxColorValue=255),
rgb(153,181,160,maxColorValue=255))

lgbt_pallete_17 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_18 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_19 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
lgbt_pallete_20 <- c(rgb(16,79,117,maxColorValue=255),
rgb(64,114,145,maxColorValue=255),
rgb(112,149,172,maxColorValue=255),
rgb(159,185,200,maxColorValue=255),
rgb(207,220,227,maxColorValue=255),
...
rgb(102,145,113,maxColorValue=255),
rgb(153,181,160,maxColorValue=255),
rgb(207,218,189,maxColorValue=255))

Terminology <- data.frame(matrix(c(
  "Women",      "refers to all self-identified women and girls, where they indicated that they were assigned female at birth in the survey.",
  "Men",        "refers to all self-identified men and boys, where they indicated that they were assigned male at birth in the survey.",
  "Cisgender",  "umbrella term that includes all men and women as defined above. We use this as we need to distinguish between respondents who are trans and those who are not to interpret the survey results, as trans respondents have a demontrably different lived experience.",
  "Trans women","refers to self-identified trans women and trans girls, or respondents who self-identified as 'woman/girl' but were assigned male at birth.",
  "Trans men",  "refers to self-identified trans men and trans boys, or respondents who self-identified as 'man/boy' but were assigned female at birth.",
  "Non-binary", "umbrella term that includes all respondents who self-identified as non-binary, genderqueer, agender or gender fluid in the survey.",
  "Trans",      "umbrella term that includes all trans women, trans men and non-binary respondents as defined above.",
  "LGBT",       "umbrella term to describe people who self-identify as lesbian, gay, bisexual, transgender or as having any other minority sexual orientation or gender identity, or as intersex.",
  "Gender aggregated", "for this category, 'Man' includes cisgender men and trans men, and 'Woman' includes cisgender women and trans women."
  ),
  ncol=2,byrow = TRUE))
colnames(Terminology) <- c("Term", "Definition")
...
Glossary <- data.frame(matrix(
  c("Characteristics","Characteristics","Age","How old are you?",
    ...
    "","","In work or education","A derived variable showing whether respondents were in work or education",
    ...
    "","","Ease of access","On a scale of 1 to 5, how easy was it for you to access specialist gender identity services in the past 12 months? If you would prefer not to answer, please leave blank.",
    "","","Barriers to access","In the past 12 months, why was accessing specialist gender identity services in the UK difficult?",
    "","","Experiences of services","Overall, how would you rate the specialist gender identity services in the UK you used in the past 12 months?",
    ...
    "","","Transitioning status","A derived variable combining two questions; At what age did you start transitioning? and To the nearest year, how long did it take to complete your transition or are you still transitioning?",
